...
# Regular expression to remove leading numbers and whitespace
def clean_minhashkey(minhashkey):
    return re.sub(r'^\d+\s*', '', minhashkey)
...
def get_column_name(minhashkey):
    return minhashkey.split('_')[1]

# Function to get row
def get_row_num(minhashkey):
    return minhashkey.split('_')[2]
